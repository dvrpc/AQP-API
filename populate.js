    ...
    res =>
      res
        ...
        .on("end", () =>
          db.serialize(() => {
            db.run(
              "CREATE TABLE IF NOT EXISTS forecasts (date TEXT PRIMARY KEY ON CONFLICT REPLACE, aqi INTEGER, forecast TEXT, updated TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"
            );
            dates
              .map(date => {
                ...
                return [
                  date,
                  Math.max(...current.map(result => result[12])),
                  current
              ...
              .forEach(vals =>
                db.run(
                  "INSERT INTO forecasts (date, aqi, forecast) VALUES (?,?,?)",
                  vals
                )
              );
            db.close();
